...
$(document).ready(function () {
  ...
  function getHum() {
    ...
    var url =
      "http://localhost/klasifikasi/raspi02/api/apiGetDataJson_ketan.php";
    $.ajax({
      type: "GET",
      url: url,
      dataType: "json",
  ...
  function predictHum(t, y) {
    ...
    for (i = 0; i < t.length; i++) {
      ...
      ft[i] =
        at[i] + bt[i] * 1 + (1 / 2) * (ct[i] * 1) * ((1 / 2) * (ct[i] * 1));
    ...
    for (var i = 0; i < 83; i++) {
      ...
      ct[lastT] =
        ((alpha * alpha) / ((1 - alpha) * (1 - alpha))) *
        (st1[lastT] - 2 * st2[lastT] + st3[lastT]);
    ...
    console.log(
      "------- SUM Absolute Values of Errordivided by Actual Values  ---------"
    );
    console.log(nAvea);
    ...
    document.getElementById("avgData9").innerHTML =
      "Mean Square Error : " + vAve / n;
    document.getElementById("avgData10").innerHTML =
      "Mean Absolute Percentage Error : " + (vAvea / n) * 100;
